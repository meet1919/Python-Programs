...
last = ['Appleyard', 'Bigmeat', 'Bloominshine', 'Boogerbottom', 'Breedslovetrout', 'Butterbaugh',
        ...
        'Wigglesworth', 'Wimplesnatch', 'Winterkorn', 'Woolysocks', 'Gruber', 'Fung', 'Labcock']

Running = True
while Running:
    enter = input('Press Enter to generate a Funny Name:')
    if enter == 'q':
        Running = False

    elif enter != 'q':
        Goofy_First = random.choice(first)
        Goofy_Last = random.choice(last)
        print('\n---%s %s---\n' % (Goofy_First, Goofy_Last))